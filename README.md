# Bala_kagitalapalli
# Hi there, I'm Bala Kagitalapalli 👋

## 💻 Full Stack Developer | MERN Stack | Spring Boot

I am a B.Tech Information Technology student passionate about building real-world web applications using modern technologies. I enjoy solving problems, learning new technologies, and creating scalable applications.

---

## 🚀 About Me

- 🎓 B.Tech - Information Technology
- 🏫 Aditya University
- 🌱 Currently learning Full Stack Development
- 💻 Passionate about MERN Stack & Spring Boot
- ☕ Java Programmer
- 🚀 Interested in Software Development and Backend Engineering

---

## 🛠 Tech Stack

<p align="center">
<img src="https://skillicons.dev/icons?i=java,javascript,react,nodejs,express,mongodb,mysql,spring,html,css,tailwind,github,vscode,vercel,render" />
</p>
### Languages
- Java
- JavaScript
- Python
- C

### Frontend
- React.js
- HTML5
- CSS3
- Tailwind CSS

### Backend
- Node.js
- Express.js
- Spring Boot

### Database
- MongoDB Atlas
- MySQL

### Tools
- GitHub
- VS Code
- Vercel
- Render
---

# 🚀 Featured Projects

## 🎯 Aditya Events Portal

A Full Stack Event Management System developed using the MERN Stack.

### Features

- Student Event Registration
- Admin Dashboard
- Dynamic Events
- Event-wise Registration Filter
- Excel Report Download
- MongoDB Atlas Integration
- Responsive Design
- Live Deployment

🔗 Live Demo:
https://aditya-events.vercel.app

---

## 📚 QR Code Based Learning Management System

A modern Learning Management System developed using React.js, Spring Boot, and MySQL.

### Features

- QR Code Based Learning
- Course Management
- Student Dashboard
- Faculty Dashboard
- Secure Login
- Responsive UI

🔗 Live Demo:
https://aditya-lms.vercel.app

---

## 📫 Connect With Me


## 📊 GitHub Statistics

<p align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Balakagitalapalli&layout=compact&theme=tokyonight"/>

</p>

📧 Email:
kagitalapallibala876@gmail.com

💼 LinkedIn:
https://www.linkedin.com/in/bala-kagitalapalli-205172291/

🐙 GitHub:
https://github.com/Balakagitalapalli

🌐 Portfolio:
https://aditya-events.vercel.app

---

⭐ Thanks for visiting my profile!
